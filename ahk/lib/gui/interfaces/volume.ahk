class VolumeInterface extends Interface {
    ...
    __New() {
        ...
        this.guiObj.MarginX := this._calcPercentHeight(0.01)
        this.guiObj.MarginY := this._calcPercentHeight(0.01)

        this.guiWidth  := this._calcPercentWidth(0.2)
        this.guiHeight := this._calcPercentHeight(0.17)
        maxHeight := this._calcPercentHeight(1)

        this.SetFont("bold s24")
        this.Add("Text", "Section Center 0x200 Background" . COLOR2 . " xm0 ym5 h" . this._calcPercentHeight(0.05) . " w" . (this.guiWidth - this._calcPercentHeight(0.02)), "Volume Control")

        volControlWidth     := this._calcPercentWidth(0.16)
        volControlHeight    := this._calcPercentHeight(0.035)
        volControlSelectBox := this._calcPercentHeight(0.01)

        ; --- ADD MASTER VOLUME CONTROL ---
        this.SetFont("norm s20")
        this.Add("Text", "Section xm0 y+" . this._calcPercentHeight(0.018), "Master")
        this.Add("Text", "vMasterControl Center f(changeVolume master) xpos1 ypos1 Background" . COLOR2 " h" . volControlHeight . " w" . volControlWidth . " y+" . this._calcPercentHeight(0.0065), "")
        ...
        this.Add("Picture", "vMasterMute f(muteVolume master) xpos2 ypos1 yp-" . (volControlSelectBox / 2) . " x+" . this._calcPercentWidth(0.012) . " h" . volControlHeight . " w" . volControlHeight
        ...
        loop programList.Length {     
            ...
            if (this.guiHeight < maxHeight) {
                newHeight := this.guiHeight + this._calcPercentHeight(0.088)
                this.guiHeight := (newHeight > maxHeight) ? maxHeight : newHeight
            }

            currProgram := globalRunning[programList[A_Index]]

            this.Add("Text", "Section xm0 y+" . this._calcPercentHeight(0.018), globalRunning[programList[A_Index]].name)
            this.Add("Text", "v" . programList[A_Index] . "Control Center f(changeVolume " . programList[A_Index] . ") xpos1 ypos" . (A_Index + 1) . " Background" . COLOR2 
                . " h" . volControlHeight . " w" . volControlWidth . " y+" . this._calcPercentHeight(0.0065), "")
            ...
            this.Add("Picture", "v" . programList[A_Index] . "Mute f(muteVolume " . programList[A_Index] . ") xpos2 ypos" . (A_Index + 1) . " yp-" . (volControlSelectBox / 2) . " x+" . this._calcPercentWidth(0.012) 
                . " h" . volControlHeight . " w" . volControlHeight, (!currProgram.muted) ? getAssetPath("icons\gui\volume.png", globalConfig) : getAssetPath("icons\gui\volume-off.png", globalConfig))
        }
    }
