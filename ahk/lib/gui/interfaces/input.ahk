class InputInterface extends Interface {
    ...
    __New() {
        ...
        this.guiWidth  := this._calcPercentWidth(0.218)
        this.guiHeight := this._calcPercentHeight(0.08)

        this.SetFont("bold s24")
        this.Add("Text", "Section Center 0x200 Background" . COLOR2 . " xm0 ym5 h" . this._calcPercentHeight(0.05) 
            . " w" . (this.guiWidth - this._calcPercentHeight(0.02)), "Input Devices")

        this.SetFont("norm s20")
        ypos := this._calcPercentHeight(0.075)
        ...
        for key, value in globalInputConfigs {
            ...
            this.Add("Text", "0x200 Background" . FONT_COLOR . " xm0 y+" . this._calcPercentHeight(0.002) . " w" . (this.guiWidth - this._calcPercentHeight(0.02)) 
                . " h" . this._calcPercentHeight(0.002), "")

            this.guiHeight += this._calcPercentHeight(0.05)

            loop globalInputStatus[key].length {
                device := globalInputStatus[key][A_Index]
        
                this.guiHeight += this._calcPercentHeight(0.05)
                ypos += this._calcPercentHeight(0.05)
        
                ; add device number text
                this.Add("Text", "0x200 xm" . this._calcPercentWidth(0.005) . " y" . ypos . " w" . this._calcPercentWidth(0.06) . " h" . this._calcPercentHeight(0.03)
                    , "Device " . A_Index)
                ...
                this.Add("Text", "v" . controlName . "Vibe f(vibe " . controlName . ") u(unvibe) Center 0x200 Background" 
                    . COLOR2 . " xpos1 ypos" . A_Index . " x" . this._calcPercentWidth(0.172) . " y" . ypos . " w" . this._calcPercentWidth(0.04) . " h" . this._calcPercentHeight(0.03) 
                    . ((!value.Has("vibration") || !value["vibration"] || !connected) ? " Hidden" : ""), "Vibe")
            }

            ypos += this._calcPercentHeight(0.025)
        }
    }
