guiInputMenu() {
    ...
    controllerInt.Add("Text", "Section Center 0x200 Background" . COLOR2 . " xm0 ym5 h" . percentHeight(0.05) 
        . " w" . (guiWidth - percentHeight(0.02)), "Input Devices")
    ...
    for key, value in globalInputConfigs {
        ...
        controllerInt.Add("Text", "0x200 xm0 y" . ypos . " w" . (guiWidth - percentHeight(0.02)) . " h" . percentHeight(0.03)
            , (value.Has("name")) ? value["name"] . "s" : "Unknown Devices")
        controllerInt.Add("Text", "0x200 Background" . FONTCOLOR . " xm0 y+" . percentHeight(0.002) . " w" . (guiWidth - percentHeight(0.02)) 
        ...
        loop globalInputStatus[key].length {
            ...
            controllerInt.Add("Text", "0x200 xm" . percentWidth(0.005) . " y" . ypos . " w" . percentWidth(0.06) . " h" . percentHeight(0.03)
                , "Device " . A_Index)
            ...
            controllerInt.Add("Text", "v" . key . "-" . A_Index . "Text Center 0x200 hp0 x" . percentWidth(0.075) . " y" . ypos 
            ...
            controllerInt.Add("Text", "v" . key . "-" . A_Index . "Outline Background" . FONTCOLOR . " hp0 x" . percentWidth(0.079) 
                . " y" . ypos . " w" . percentWidth(0.075) . ((connectionType != 1) ? " Hidden" : ""), "")
            controllerInt.Add("Progress", "v" . key . "-" . A_Index . "Progress Background" . COLOR1 . " c" . batteryColor . " yp+" . (borderWidth / 2) 
                . " xp+" . (borderWidth / 2) . " wp-" . borderWidth . " hp-" . borderWidth . ((connectionType != 1) ? " Hidden" : ""), batteryLevel)
            controllerInt.Add("Text", "v" . key . "-" . A_Index . "Nub Background" . FONTCOLOR . " x+" . (borderWidth - percentWidth(0.002)) 
                . " y" . (ypos + percentHeight((0.03 - 0.01) / 2)) . " w" . percentWidth(0.004) . " h" . percentHeight(0.01) . ((connectionType != 1) ? " Hidden" : ""), "")
    
            ; add vibe button
            controllerInt.Add("Text", "v" . key . "-" . A_Index . "Vibe Center 0x200 Background" . COLOR2 . " xpos1 ypos" . A_Index . " x" . percentWidth(0.172) 
                . " y" . ypos . " w" . percentWidth(0.04) . " h" . percentHeight(0.03) . ((!value.Has("vibration") || !value["vibration"] || !connected) ? " Hidden" : ""), "Vibe")
        }
...
; causes the currently selected input device to vibrate/stop
...
ControllerSecondTimer() {
    ...
    for key, value in globalInputConfigs {
        ...
        loop globalInputStatus[key].length {
            ...
            ; update values
            currGui[key . "-" . A_Index . "Text"].Text := connectionText
            ...
            currGui[key . "-" . A_Index . "Progress"].Value := batteryLevel
            currGui[key . "-" . A_Index . "Progress"].Opt("c" . batteryColor)

            ; update visibility status
            currGui[key . "-" . A_Index . "Text"].Visible := connectionType != 1 || !connected
            currGui[key . "-" . A_Index . "Outline"].Visible := connectionType = 1 && connected
            currGui[key . "-" . A_Index . "Progress"].Visible := connectionType = 1 && connected
            currGui[key . "-" . A_Index . "Nub"].Visible := connectionType = 1 && connected
            currGui[key . "-" . A_Index . "Vibe"].Visible := connected
        }
    }
