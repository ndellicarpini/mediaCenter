...
; creates the global.cfg if it doesn't exist (copying global.default.cfg), and tells the user to
...
readGlobalConfig() {
	...
	if (FileExist("global.cfg")) {
		gConfig := readConfig("global.cfg",, "brackets")
		...
		; if there is no global.cfg or global.default.cfg, you have to find them
		if (!FileExist("global.default.cfg")) {
			ErrorMsg(
				(
					"
					There are literally no config files in config\
					No global.cfg & No global.default.cfg
		...
		defaultGlobal := FileOpen("global.default.cfg", "r")
		defaultContents := defaultGlobal.Read()
		defaultGlobal.Close()

		newGlobal := FileOpen("global.cfg", "w")
